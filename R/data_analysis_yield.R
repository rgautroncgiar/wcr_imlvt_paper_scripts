...
library(asreml)
coffee <- read_excel("source_data/Copy of IMLVTGlobal_YieldperYear_V16Jul24.xlsx")

plant_date <- read_excel("source_data/planting_dates.xlsx") %>% select(-num)

colnames(coffee) <- c("site", "variety", "block", "YLDPlant", "year")

colnames(plant_date) <- c("site", "variety", "original_date", "N_plants") 

# join plant date to coffee data set
coffee <- coffee %>% left_join(plant_date, by = c("site" = "site", 
                                        "variety" = "variety"))

#choose years greater equal than year_3
coffee_year_int <- coffee %>% mutate(year_planting = year(original_date),
                  year_3 = year_planting + 3) %>% 
  filter(year>= year_3) %>% select(site, variety, block, YLDPlant, year) %>% 
  group_by(site, variety, block) %>% 
  summarise(yield = sum(YLDPlant))

coffee_year_int <- 
  coffee_year_int %>%
  group_by(site, variety) %>%
  complete(block = 1:3) %>%   # fill yield with NA values
  ungroup()

coffee_year_int %>% ggplot(aes(x = reorder(site, yield), y = yield, fill = factor(block))) +
  geom_boxplot() +
  theme_xiaofei()

coffee_year_int <- coffee_year_int %>%  
  mutate(yield = ifelse(site == "Copan_HND" & 
                          variety == "IPR103" & 
                          block == 1, NA, yield))

#saveRDS(my_data, "data.rds")
#saveRDS(coffee_year_int, "data/coffee_year_int_yield.rds")

#coffee_year_int %>% filter(variety == "BP429A") # no existe en rendimiento

sites <- unique(coffee_year_int$site)

#pdf("images/raw_data_by_site.pdf", width = 10, height = 6)
for (s in sites) {
  cat("Procesando sitio:", s, "\n")
  
  # Filtrar por sitio
  df_site <- coffee_year_int %>% filter(site == s)
...
master_data[["raw_data"]] <- coffee_year_int 

var <- "yield"
summ_design <- coffee_year_int %>%
...
h2_all <- h2_all %>% left_join(h2_all_cullis) %>% clipr::write_clip()
...
# joining blues + weights
blue_all <- blue_all %>% 
  mutate(site = as.factor(site))

# connectivity plot
connect_mat <- check_connectivity(data = coffee_year_int, 
                   genotype = "variety",
                   trial = "site",
                   response = "yield",
...
#pdf(paste("images/connec_yield_fa2", Sys.Date(), sep = "_", ".pdf"), width = 8, height = 6) 
...
# ggsave(paste("images\\connectivity_plot", ".png"),
#        units = "in", dpi = 300, width = 10, height = 8)

# remove from the single analysis bugusege_UGA (low connectivity) and COPAN (low heritability)

blues_full = blue_all[!blue_all$site %in% c("Bugusege_UGA", "Copan_HND"), ] 

#saveRDS(blues_full, "data/blues_full_yield_4GxE.rds")
#blues_full <- readRDS("data/blues_full_yield_4GxE.rds")

# Fit a multi-environment model with FA and a simple error structure using ~ units
# BLUEs were already weighted, and the site-specific variances
# were partially absorbed by the errors in the univariate model
...
# model assumptions
#pdf(paste("images/plot_assumption_yiedl", Sys.Date(), sep = "_", ".pdf"), width = 6, height = 6) 
plot(mta_fa_model)
#dev.off()
...
# We need to know which model to choose based on AIC and BIC
# factor analytics 1
mta_fa_model_1 <- asreml(
  fixed = BLUE ~ site,
  random = ~ fa(site, 1):variety,
  residual = ~ units,
  weights = weight,
...
mta_fa_model_1 <- update.asreml(mta_fa_model_1)
mta_fa_model_1 <- update.asreml(mta_fa_model_1)
mta_fa_model_1 <- update.asreml(mta_fa_model_1)
mta_fa_model_1 <- update.asreml(mta_fa_model_1)

aic_1 <- summary(mta_fa_model_1)$aic
bic_1 <- summary(mta_fa_model_1)$bic

# factor analytics 3
mta_fa_model_3 <- asreml(
  fixed = BLUE ~ site,
...
mta_fa_model_3 <- update.asreml(mta_fa_model_3)
mta_fa_model_3 <- update.asreml(mta_fa_model_3)
mta_fa_model_3 <- update.asreml(mta_fa_model_3)
mta_fa_model_3 <- update.asreml(mta_fa_model_3)
mta_fa_model_3 <- update.asreml(mta_fa_model_3)
...
aic_3 <- summary(mta_fa_model_3)$aic
bic_3 <- summary(mta_fa_model_3)$bic
...
# Standard errors and CI by genotype:
...
blups_site_var <- predict(mta_fa_model, classify = "variety:site")$pvals %>%
  group_by(variety) %>%
  summarise(
    stability = var(predicted.value, na.rm = T), # Menor varianza = más estable
...
#meta_file_name <- paste0(folder_output, paste("comments", "yield", "master_results", Sys.Date(), ".xlsx", sep = "_"))
#write.xlsx(comments_review, file = meta_file_name)

# extract variance components from Fa1 model
vars <- lucid::vc(mta_fa_model)

# extract loadings or factors
fa1.loadings =  vars[grepl("!fa1$", vars$effect), "component"]
fa2.loadings =  vars[grepl("!fa2$", vars$effect), "component"]
...
VarTot # The percentage of variance explained by the 2 factors is 57.74%
...
library(pheatmap)
# png(filename = "images/Fa2/corr_sites_fa2_yield.png",
...
# Vector PDF output
# pdf(file = "images/Fa2/corr_sites_fa2_yield.pdf",
#     width = 8, height = 8,
...
gg_cor(Cmat,
       show_signif = F)
# ggsave(paste("images\\corr_sites_fa2", ".png"),
#        units = "in", dpi = 300, width = 12, height = 6)

# Extract all fixed effects from the model
...
# pdf(paste("images/Loadings", Sys.Date(), sep = "_", ".pdf"), width = 8, height = 8) 

loadings

#dev.off()

# ggsave(paste("images\\loadings", ".png"),
#        units = "in", dpi = 300, width = 12, height = 6)
...
library(patchwork)
# Abrir dispositivo PDF
#pdf(paste("images/latent_reg_plot_sites", Sys.Date(), sep = "_", ".pdf"), width = 12, height = 8) 

p1 + p2 + plot_layout(guides = "collect")
#dev.off()
# ggsave(paste("images\\latent_reg_plot_sites", ".png"),
#        units = "in", dpi = 300, width = 14, height = 6)

# gen_fa_scores <- summary(mta_fa_model, coef = TRUE)$coef.random
# fa_scores <- gen_fa_scores[grep("fa1|fa2", rownames(gen_fa_scores)), ]
...
#write.csv(fa12_scores, "varieties_scores_fa_yield.csv", row.names = F)
...
# ggsave(paste("images\\latent_reg_biplot", ".png"),
...
# ggsave(paste("images\\circle_cor_loadings", ".png"),
...
# prediction based on factors
# using the rotation
L.star %*% f.star[rownames(f.star)=="AB3"]

# non using the rotation
L %*% f[rownames(f)=="AB3"]
...
for (acc in accessions) {
  ...
  geom_smooth(aes(x=value,y=regblup), method = "lm", formula = y~x, se = F)+
  ggpubr::stat_regline_equation(
    aes(x = value, y = regblup, label = paste(..rr.label.., sep = "~~~")),
    size = 5,
    label.x.npc = 0.5,
    label.y.npc = 1
  ) +
...
#saveRDS(FAST, "FAST.rds")
#FAST <- readRDS("FAST.rds")

# Fix variety name
FAST <- FAST %>% mutate(Genotype = recode(Genotype, "Kartila1" = "Kartika1")) 

#pdf("images/Fa2/latent_plots_yiedl.pdf", width = 12, height = 8)
...
# save excel file results
# folder_output <- here::here("output//")
# meta_file_name <- paste0(folder_output, paste("2025", "WCR", "master_results_yield", Sys.Date(), ".xlsx", sep = "_"))
# write.xlsx(master_data, file = meta_file_name)

# other way to plot Fa vs value
accessions <- unique(FAST$Genotype)
pdf("images/latent_plot_by_acc_yield.pdf", width = 10, height = 6)
...
for (acc in accessions) {
  ...
  p <- df_acc %>%
    ...
    facet_grid(Genotype ~ loading, scales = "free_x") +
    theme_bw(base_size = 15) +
    geom_smooth(aes(y = blup), method = "lm", formula = y ~ x, se = FALSE, color = "blue") +
    geom_label_repel(aes(label = site), nudge_y = 0.05, nudge_x = -0.03, force = 1) +
    geom_hline(yintercept = 0, linetype = 2, color = "grey") +
